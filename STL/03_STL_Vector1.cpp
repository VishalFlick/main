//TOPIC: std::vector

//Syntax: std::vector<T>vector;

//NOTICE:
// 1. std::vector is a sequence container and also known as Dynamic Array or Array List.
// 2. Its size can grow and shrink dynamically, and no need to provide size at compile time.

// ELEMENT ACCESS
// at(), [], front(), back(), data()

// MODIFIERS:
// insert(), emplace(), push_back(), emplace_back(), pop_back(), reserve()
// resize(), swap(), erase(), clear()

#include<iostream>
#include<vector>
using namespace std;


int main()
{
    // Declarations
    std::vector<int>myVec1;
    std::vector<int> myVec2(5, 20);
    std::vector<int>myVec3 = {1, 2, 3, 4, 5}; // initializer list
    std::vector<int>myVec4 {1, 2, 3, 4, 5}; // unifrom initialization

    // Accessing Elements
    myVec2[3] = 10;
    myVec2.at(3) = 10;

    return 0;
}
